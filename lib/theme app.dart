import 'package:flutter/material.dart';
import 'package:google_fonts/google_fonts.dart';

class ThemeApp {
  static const Color darkGreen = Color(0xFF519872);
  static const Color green = Color.fromARGB(255, 142, 210, 173);
  static const Color orange = Color.fromARGB(255, 255, 218, 133);
  static const Color darkOrange = Color(0xFFEBA90D);
  static const Color white = Colors.white;
  static const Color black = Colors.black;
  static const Color whiteGray = Color.fromARGB(255, 231, 228, 228);
  static const Color gray=Color(0xFF999999);
  static var font = GoogleFonts.ibmPlexSansArabic();
  static const Color isDarkModeMian=Color.fromARGB(255, 0, 0, 0);
  static const Color delete =Color.fromARGB(255, 194, 30, 30);

  static const Color isDarkMode =Color.fromARGB(255, 33, 52, 42); 

}
